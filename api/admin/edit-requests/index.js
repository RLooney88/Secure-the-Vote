...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { SITE_ID, SITE_KEY, callbackUrl, sendToNova, splitAssets, uuid, PUBLIC_SITE_URL } = require('../edit-requests-helpers');
      ...
      const insert = await pool.query(
        ...
        [clientRequestId, body.title, body.page || '', body.description, body.requester_email || '', JSON.stringify(body.attachments || []), body.preview_approval_needed !== false, JSON.stringify([{ action: 'created', timestamp: new Date().toISOString(), user: admin.email || admin.id }])]
      );
      const row = insert.rows[0];
      const { images, files } = splitAssets(body.attachments || []);
